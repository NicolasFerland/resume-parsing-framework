    ...
    def __init__(self):
        ...
        try:
            ...
            if selected_model not in available_models:
                ...
                selected_model = next((m for m in available_models if 'gemini' in m), None)
            
            if selected_model:
                self.model = genai.GenerativeModel(selected_model)
                logger.info(f"LLM initialized successfully with model: {selected_model}")
            else:
...
class NameExtractor(BaseLLMExtractor):
    ...
    def extract(self, text: str) -> str:
        ...
        # This reduces token costs while maintaining high accuracy for name identification
        context = text[:1000]
        prompt = (
            "Identify the full name of the candidate from this resume text. "
            "The name is usually at the top, but look for standard clues like larger fonts or primary headers. "

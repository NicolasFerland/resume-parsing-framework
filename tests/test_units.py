...
def test_llm_list_models_failure(mocker):
    ...
    mocker.patch(
        "extractors.llm_strategies.genai.list_models",
...
def test_resume_parser_framework_empty_text(mocker):
    ...
    try:
        ...
        mock_coordinator.orchestrate.return_value = ResumeData(
            name="Unknown", email="Not Found", skills=[]
        )
...
def test_coordinator_name_exception(mocker):
    ...
    extractor = ResumeExtractor(
        {"name": mock_name, "email": mock_email, "skills": mock_skills}
    )
...
def test_llm_no_available_models(mocker):
    ...
    mocker.patch(
        "extractors.llm_strategies.genai.list_models", return_value=[mock_model]
    )
...
def test_llm_default_credentials_error(mocker):
    ...
    mocker.patch(
        "extractors.llm_strategies.genai.configure",
        side_effect=DefaultCredentialsError("Auth error"),
    )
...
def test_email_extractor_exception(mocker):
    ...
    mocker.patch(
        "extractors.text_strategies.re.search", side_effect=Exception("Regex error")
    )
...
def test_pdf_parser_read_error(mocker):
    ...
    mock_reader = mocker.patch("pypdf.PdfReader")
    mock_reader.side_effect = mocker.MagicMock(side_effect=Exception("PdfReadError"))

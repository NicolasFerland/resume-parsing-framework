...
def test_unsupported_extension(mocker):
    ...
    framework = ResumeParserFramework(
        parsers={".pdf": mock_parser}, extractor_coordinator=mock_coord
    )
...
def test_llm_initialization_failure(mocker):
    ...
    mocker.patch(
        ...
        side_effect=DefaultCredentialsError("Invalid key"),
    )
...
def test_parser_corruption_error(mocker):
    ...
    mocker.patch(
        "docx2txt.process", side_effect=Exception("Corrupted document structure")
    )
